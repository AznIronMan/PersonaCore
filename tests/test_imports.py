...
def test_personaconsole_is_canonical_api():
    ...
    assert personaconsole.INFRASTRUCTURE_POSTURE_FEATURE == "infrastructure_posture"
    ...
    assert personaconsole.PUBLIC_PROFILE_FEATURE == "public_profile"
    ...
    assert personaconsole.PRESENCE_MONITOR_FEATURE == "presence_monitor"
    assert personaconsole.RUNTIME_TASK_BOARD_FEATURE == "runtime_task_board"
    ...
    assert personaconsole.SystemSecretFilterState
    ...
    assert "render_public_profile_surface" in personaconsole.__all__
    ...
    assert "render_infrastructure_posture_surface" in personaconsole.__all__
    ...
    assert "render_presence_monitor_surface" in personaconsole.__all__
    assert "render_runtime_task_board_surface" in personaconsole.__all__
...
def test_legacy_import_shims_reexport_canonical_api():
    for legacy in (persona_console, personacore):
        ...
        assert legacy.render_infrastructure_posture_surface is personaconsole.render_infrastructure_posture_surface
        ...
        assert legacy.render_public_profile_surface is personaconsole.render_public_profile_surface
        ...
        assert legacy.render_presence_monitor_surface is personaconsole.render_presence_monitor_surface
        ...
        assert legacy.SystemSecretInventoryRow is personaconsole.SystemSecretInventoryRow
...
def test_legacy_submodules_reexport_canonical_implementation():
    legacy_modules = {
        ...
        "infrastructure_posture": "render_infrastructure_posture_surface",
        "worker_operations": "render_worker_operations_surface",
        ...
        "presence_monitor": "render_presence_monitor_surface",
        "runtime_task_board": "render_runtime_task_board_surface",
        "privacy": "render_private_text",
        "public_presence": "render_public_splash_page",
        "public_profile": "render_public_profile_surface",
...
def test_public_package_metadata_matches_runtime_version():
    ...
    assert pyproject["project"]["version"] == personaconsole.__version__ == "1.0.51"
