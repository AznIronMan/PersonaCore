...
- Added `FlashBanner`, `render_flash_banners(...)`, `flash_query_params(...)`,
  and `flash_url(...)`.
- Added `personaconsole.controls` exports for the flash helpers under the public
  import path.
- The flash renderer emits both PersonaConsole classes (`pc-flash-*`) and
...
PersonaConsole does not perform redirects, mutate runtime state, authorize actions,
...
PYTHONPATH=src python3 examples/fixture_app.py --output /tmp/personaconsole-fixture-1.0.16.html
